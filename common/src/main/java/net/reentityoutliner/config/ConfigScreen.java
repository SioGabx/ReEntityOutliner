    ...
    protected void init() {
        ...
        searchField = new EditBox(this.font, this.width / 2 - 100, 6, 200, 20, Component.literal(searchText)) {
            ...
            public boolean mouseClicked(@NonNull MouseButtonEvent event, boolean doubleClick) {
                var val = super.mouseClicked(event, doubleClick);
        ...
        this.addRenderableWidget(Button.builder(Component.literal("?"), (_) -> Util.getPlatform().openUri("https://github.com/SioGabx/ReEntityOutliner/wiki/How-to-search"))
        ...
        if (!results.isEmpty()) {
            if (groupByCategory) {
                ...
                for (EntityType<?> entityType : results) {
                    resultsByCategory.computeIfAbsent(entityType.getCategory(), _ -> new ArrayList<>()).add(entityType);
                }
